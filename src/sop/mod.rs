...
1. **Check readings** — Read sensor data and confirm.
   - tools: sensor_read, memory_store

2. **Close valve** — Set valve output LOW.
   - tools: actuator_write, sensor_read
...
"#;
        ...
        assert_eq!(
            steps[1].suggested_tools,
            vec!["actuator_write", "sensor_read"]
        );
...
"#,
    ...
    fn extract_bold_title_with_dash() {
        let (title, body) = extract_bold_title("**Close valve** — Set valve output LOW.").unwrap();
        assert_eq!(title, "Close valve");
        assert_eq!(body, "Set valve output LOW.");
    }

    #[test]
    fn extract_bold_title_no_separator() {
        let (title, body) = extract_bold_title("**Close valve** Set output LOW.").unwrap();
        assert_eq!(title, "Close valve");
        assert_eq!(body, "Set output LOW.");
    }
