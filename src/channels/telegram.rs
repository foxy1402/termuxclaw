...
const TELEGRAM_INLINE_PDF_MAX_CHARS: usize = 8_000;
const TELEGRAM_INLINE_TEXT_MAX_CHARS: usize = 12_000;
...
Allowlist Telegram username (without '@') or numeric user ID.",
    ...
    async fn extract_pdf_text_snippet(bytes: Vec<u8>) -> Option<String> {
        #[cfg(feature = "rag-pdf")]
        {
            ...
            .await
            {
                Ok(Ok(text)) => text,
                Ok(Err(err)) => {
                    tracing::debug!("telegram: pdf extraction failed: {err}");
                    return None;
                }
                Err(err) => {
                    tracing::debug!("telegram: pdf extraction task failed: {err}");
                    return None;
                }
            };
    ...
    fn extract_plain_text_snippet(bytes: &[u8]) -> Option<String> {
        ...
        let mut output: String = trimmed
            .chars()
            .take(TELEGRAM_INLINE_TEXT_MAX_CHARS)
            .collect();
        if trimmed.chars().count() > TELEGRAM_INLINE_TEXT_MAX_CHARS {
            let _ = write!(
                output,
                "\n\n... [truncated at {} chars]",
                TELEGRAM_INLINE_TEXT_MAX_CHARS
            );
        }
    ...
    async fn try_parse_attachment_message(
        ...
        if matches!(attachment.kind, IncomingAttachmentKind::Document)
            && is_pdf_extension(&local_path)
        {
            #[cfg(feature = "rag-pdf")]
            {
                if let Some(extracted_text) =
                    Self::extract_pdf_text_snippet(file_data.clone()).await
                {
                    let _ = write!(content, "\n\n[Extracted PDF text]\n{}", extracted_text);
                }
            }
            ...
            #[cfg(not(feature = "rag-pdf"))]
            {
                let _ = write!(
                    content,
                    "\n\n[PDF extraction unavailable in this build. Rebuild with: cargo build --features rag-pdf]"
                );
            }
        }

        if matches!(attachment.kind, IncomingAttachmentKind::Document)
            && is_text_extension(&local_path)
        {
            if let Some(extracted_text) = Self::extract_plain_text_snippet(&file_data) {
                let _ = write!(content, "\n\n[Extracted text]\n{}", extracted_text);
            }
        }
...
mod tests {
    ...
    #[test]
    fn is_pdf_extension_detects_pdf_only() {
        assert!(is_pdf_extension(std::path::Path::new("report.pdf")));
        assert!(is_pdf_extension(std::path::Path::new("REPORT.PDF")));
        assert!(!is_pdf_extension(std::path::Path::new("report.txt")));
        assert!(!is_pdf_extension(std::path::Path::new("report")));
    }

    #[test]
    fn is_text_extension_detects_plain_text_types() {
        assert!(is_text_extension(std::path::Path::new("menu.txt")));
        assert!(is_text_extension(std::path::Path::new("notes.md")));
        assert!(is_text_extension(std::path::Path::new("data.csv")));
        assert!(is_text_extension(std::path::Path::new("config.json")));
        assert!(!is_text_extension(std::path::Path::new("scan.pdf")));
        assert!(!is_text_extension(std::path::Path::new("file")));
    }

    #[test]
    fn extract_plain_text_snippet_handles_utf8_and_trims() {
        let bytes = b"\n\n  hello menu\nline 2\n";
        let extracted =
            TelegramChannel::extract_plain_text_snippet(bytes).expect("expected extracted text");
        assert!(extracted.starts_with("hello menu"));
