...
impl SecurityPolicy {
    ...
    pub fn is_command_allowed(&self, command: &str) -> bool {
        ...
        // Personal Termux mode: wildcard allowlist with no risk blocking means
        // ALL commands are permitted — skip every extra guard.
        let has_wildcard = self.allowed_commands.iter().any(|c| c.trim() == "*");
        if has_wildcard && !self.block_high_risk_commands {
            // Still need at least one non-empty token to be a valid command.
            return command.split_whitespace().next().is_some();
        }
...
mod tests {
    ...
    fn default_policy_has_sane_values() {
        ...
        // Personal Termux: fully open by default
