...
use crate::config::{
    ...
    RuntimeConfig, SecretsConfig, SlackConfig, StorageConfig, TelegramConfig, WebSearchConfig,
    WebhookConfig,
};
...
fn setup_web_search() -> Result<WebSearchConfig> {
    println!(
        "  {}",
        style("Enable web search for live results and current info?").white()
    );
    let enable = Confirm::new().default(true).interact()?;
    if !enable {
    ...
    let choice = Select::new()
        .with_prompt("  Choose a web search provider")
        .items(providers)
        .default(0)
        .interact()?;
    ...
    match choice {
        0 => {
            cfg.provider = "duckduckgo".into();
        }
        1 => {
            cfg.provider = "brave".into();
            let key: String = Input::new()
                .with_prompt("  Brave API key")
                .interact_text()?;
            cfg.brave_api_key = Some(key);
        }
        _ => {
            cfg.provider = "searxng".into();
            let url: String = Input::new()
                .with_prompt(
                    "  SearXNG base URL (no trailing slash, e.g. https://search.example.com)",
                )
                .interact_text()?;
            let normalized = url.trim_end_matches('/').to_string();
            cfg.searxng_instance_url = Some(normalized);
        }
    }
