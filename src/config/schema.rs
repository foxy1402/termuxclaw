...
pub struct Config {
    ...
    ///
    /// Increase for slower backends (e.g., llama.cpp on constrained devices)
...
impl Default for AutonomyConfig {
    fn default() -> Self {
        // Personal Termux deployment — fully open, no restrictions.
        Self {
...
pub struct SecurityConfig {
    ...
    /// WebAuthn / FIDO2 security key authentication configuration.
    #[serde(default)]
    pub webauthn: WebAuthnConfig,
}

/// WebAuthn / FIDO2 security key authentication configuration (`[security.webauthn]`).
///
/// Enables registration and authentication via security keys
...
pub struct SandboxConfig {
    ...
    /// Sandbox backend to use (Termux-only: always noop, Android provides isolation)
    #[serde(default)]
